...
const HomeContentPreview: React.FC<HomeContentPreviewProps> = ({ content, onEdit, onBack }) => {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Home Content Preview</h1>
              <p className="text-slate-400">Preview how your content will appear</p>
            </div>
          </div>
        ...
        <article className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
          ...
          <div className="p-8">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(content.status)}`}>
                  <Clock className="w-3 h-3" />
                  {content.status.charAt(0).toUpperCase() + content.status.slice(1)}
                </div>
                {content.featured && (
                  <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium text-yellow-400 bg-yellow-400/10">
                    <span>★</span>
                    Featured
                  </div>
                )}
            ...
            {content.pages && content.pages.length > 0 && (
              <div className="flex items-center gap-2 mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {content.pages.map((page: string) => (
                    <span key={page} className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full text-sm">
              ...
              <div className="mt-8 pt-6 border-t border-slate-700">
                <div className="flex items-center gap-2 text-slate-400">
                  ...
                  <span className="text-sm">
                    {content.status === 'published' ? 'Published on' : 'Scheduled for'}: {formatDate(content.publishDate)}
                  </span>
                </div>
              </div>
            )}
