...
export default function UpdateInitiatives() {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKEND_URL}/api/initiatives`)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post(`${import.meta.env.VITE_BACKEND_URL}/api/initiatives`, formData)
      .then(() => alert('✅ Initiatives updated successfully.'))
      .catch((err) => {
        console.error('❌ Submit Error:', err);
        alert('❌ Failed to update initiatives.');
      });
  };

  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center mb-8">🚀 Update Initiatives</h1>
      <form onSubmit={handleSubmit}>
        {formData.map((item, index) => (
          <div key={index} className="bg-white border rounded-lg p-6 mb-6 shadow">
            <h2 className="text-xl font-semibold mb-4">Initiative {index + 1}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Write the full details or description..."
                ></textarea>
              </div>
            </div>
        ...
        <div className="flex justify-between items-center mt-6">
          ...
          >
            {uploading ? 'Uploading...' : '💾 Save All Initiatives'}
          </button>
        </div>
      </form>
    </div>
