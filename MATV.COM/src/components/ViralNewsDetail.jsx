...
export default function ViralNewsDetail() {
  const { id } = useParams(); // can be id or slug
  const navigate = useNavigate();
  const [story, setStory] = useState(null);
  const { fetchOne: fetchFallbackOne } = useGeneralNewsFallback();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // Try your primary backend first
        const res = await blogPublicService.getPublishedContentOnPagesById('ViralNews', id);
        if (!cancelled && res) {
          setStory({
            ...
            category: res.category || 'Viral News',
        ...
        console.error('❌ Backend viral detail failed, using fallback...', err);
      }

      // Fallback – use General News store (sub_category_name filter)
      try {
        const item = await fetchFallbackOne(id);
        if (!cancelled && item) setStory(item);
      } catch (fbErr) {
        console.error('❌ Fallback viral detail failed:', fbErr);
      }
    };
