...
const ScienceNews = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const contentRes = await blogPublicService.getPublishedContentOnPages('ScienceNews');
        setNews(contentRes.contents || []);

        const blogRes = await blogPublicService.getPublishedBlogOnPages('ScienceNews');
  ...
  return (
    <>
      ...
      <section className="bg-white py-12 px-4 max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-10">🔬 Science News</h2>

        {news.length === 0 ? (
          <p className="text-center text-gray-500">No science news available.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {news.map((item, idx) => (
              <Link
                to={`/science-news/${item.slug || item._id}`}
                key={idx}
      ...
      <section className="bg-gray-50 py-12 px-4 max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-10">🧪 Science Blog</h2>
