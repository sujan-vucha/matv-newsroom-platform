...
export default function UpdateIndiaNews() {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKEND_URL}/api/india-news`)
      .then((res) => setFormData(Array.isArray(res.data) ? res.data : []))
      .catch((err) => console.error('❌ Failed to load India news:', err));
  }, []);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post(`${import.meta.env.VITE_BACKEND_URL}/api/india-news`, formData)
      .then(() => alert('✅ India news updated'))
      .catch((err) => {
        console.error('Submit Error:', err);
        alert('❌ Failed to update India news');
      });
  };

  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center mb-8">🇮🇳 Update India News</h1>
