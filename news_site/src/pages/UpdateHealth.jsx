...
export default function UpdateHealth() {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKEND_URL}/api/health`)
      .then((res) => setFormData(Array.isArray(res.data) ? res.data : []))
      .catch((err) => console.error('Failed to load health articles', err));
  }, []);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post(`${import.meta.env.VITE_BACKEND_URL}/api/health`, formData)
      .then(() => alert('✅ Health articles updated'))
      .catch((err) => {
        console.error('Submit Error:', err);
        alert('❌ Failed to update health articles');
      });
  };

  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-center mb-8">💊 Update Health Articles</h1>
